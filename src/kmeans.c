...
int* cluster_angles( float* angles, const int size){
	...
	config.num_objs = size;
	config.k = 2;
	config.max_iterations = 100;
	config.distance_method = d_distance;
	config.centroid_method = d_centroid;

  	config.objs = calloc(config.num_objs+2, sizeof(Pointer));
    ...
    //memcpy(config.centers[i], c[i], sizeof(c[i]))
	}

	/* run k-means */
	kmeans_result = kmeans(&config);

	/* print result */
	for (i = 0; i < config.num_objs; i++){
    	result[i] = config.clusters[i];
    //memcpy(result[i], config.clusters[i], sizeof(result[i]))
	}
